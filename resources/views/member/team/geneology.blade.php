...
@section('container')
                ...
                <div class="genealogy-tree-stage" id="genealogyTreeStage">
                    <div class="genealogy-root-zone">
                        <div class="genealogy-root-card">
                            <div class="genealogy-root-avatar {{ $rootAvatarClass }}">
                                    ...
                                    @foreach ($level['members'] as $member)
                                        @php
                                            ...
                                            $memberImage = is_object($member)
                                                ...
                                                : $member['profile_image'] ?? '';
                                            $memberEmail = is_object($member)
                                                ? $member->email ?? ''
                                                : $member['email'] ?? '';
                                            $memberCreated = is_object($member)
                                                ? $member->created_at ?? ''
                                                : $member['created_at'] ?? '';
    ...
    <script>
        ...
        function applyGenealogySearch() {
            ...
            levelWraps.forEach((wrap) => {
                ...
                cards.forEach((card) => {
                    ...
                    card.closest('.genealogy-node-link').style.display = match ? '' : 'none';
                    if (match) {
                        visibleCount++;
                    }
                });
        ...
        document.getElementById('genealogy-search-input')?.addEventListener('input', applyGenealogySearch);
