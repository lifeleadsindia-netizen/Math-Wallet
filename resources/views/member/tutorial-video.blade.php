...
@section('container')
        ...
        <div class="container-fluid pt-2 pb-5" style="padding-bottom: 80px !important;">
            ...
            <div class="page-titles mb-3">
                <div class="welcome-text">
                    <h4 class="text-white font-weight-bold mb-1">Tutorial Videos</h4>
                ...
                <div class="row">
                    <div class="col-12 text-center py-5">
                        <div class="p-5 rounded-4 side-panel-card text-center" style="border: 1px dashed rgba(255,255,255,0.2);">
                            ...
                            <h4 class="text-white fw-bold mb-2">No Tutorial Videos Available Yet</h4>
                            <p class="text-white-50 mb-0" style="font-size: 14px;">Official step-by-step tutorial guides will appear here once published by admin.</p>
                        </div>
                    </div>
                </div>
            @else
                <!-- Video Theater & Side Info Section -->
                <div class="row g-4 mb-4" id="tutTheaterSection">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                <div class="d-flex align-items-center gap-2">
                                    <span class="badge tutorial-badge">
                                        <i class="fas fa-graduation-cap me-1"></i> <span id="theaterTag">{{ $video['tag'] }}</span>
                                    </span>
                                ...
                                <span class="text-white-50" style="font-size: 12px;" id="theaterDuration">
                                    <i class="fas fa-clock text-info me-1"></i> {{ $video['duration'] }}
                                </span>
                            </div>

                            <!-- Video Container (Responsive 16:9 Aspect Ratio) -->
                            <div class="tutorial-player-wrapper rounded-3 overflow-hidden shadow-lg position-relative mb-3" style="background: #000;">
                                    ...
                                    <video id="mainTutorialVideo" controls playsinline preload="metadata"
                                        ...
                                        <p class="text-white p-4 text-center">
                                            ...
                                            @if($video['can_download'])
                                                <a href="{{ $video['download_url'] }}" class="btn btn-info btn-sm ms-2">Download Video</a>
                                            @endif
                                        </p>
                                    </video>
                    ...
                    <!-- Right: Quick Referral Link & Statistics -->
                    <div class="col-xl-4 col-lg-5 col-md-12">
                        <div class="d-flex flex-column gap-4">
                            <!-- Quick Referral Link Card -->
                            <div class="tutorial-side-card p-3 p-xl-4">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    <h6 class="text-white fw-bold mb-0" style="font-size: 14px;">
                                        <i class="fas fa-share-alt text-info me-2"></i> Your Referral Link
                                    </h6>
                                    <span class="badge bg-info-subtle text-info" style="font-size: 10.5px;">Active</span>
                                </div>
                                <p class="text-white-50 mb-3" style="font-size: 12px; line-height: 1.5;">
                                    Share this tutorial together with your referral link so new members understand the steps and
                                    join your team immediately.
                                </p>
                                ...
                                <div class="position-relative mb-2">
                                    ...
                                    <input type="text" id="memberReferralInput" class="form-control tutorial-referral-input ps-5"
                                        value="{{ $referralLink }}" readonly>
                                </div>

                                <div class="d-grid gap-2">
                                    <button type="button"
                                        class="btn btn-info btn-sm fw-bold d-flex align-items-center justify-content-center text-white"
                            ...
                            <div class="tutorial-side-card p-3 p-xl-4">
                                <h6 class="text-white fw-bold mb-3 d-flex align-items-center" style="font-size: 14px;">
                                    <i class="fas fa-graduation-cap text-success me-2"></i> Tutorials Library
                                </h6>
                                <ul class="list-unstyled mb-0 d-flex flex-column gap-2 text-white-50" style="font-size: 12.5px;">
                                    <li class="d-flex justify-content-between align-items-center py-1 border-bottom border-secondary-subtle">
                                        <span><i class="fas fa-film me-2 text-muted"></i> Total Tutorial Guides</span>
                                        <span class="badge bg-info">{{ count($allVideos) }} Guides</span>
                                    </li>
                                    <li class="d-flex justify-content-between align-items-center py-1 border-bottom border-secondary-subtle">
                                        <span><i class="fas fa-tv me-2 text-muted"></i> Streaming Format</span>
                                        <span class="badge bg-success-subtle text-success">1080p HD Video</span>
                                    </li>
                                    <li class="d-flex justify-content-between align-items-center py-1">
                                        <span><i class="fas fa-check-double me-2 text-muted"></i> System Coverage</span>
                                        <span class="text-white fw-bold">Full Platform</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <!-- ALL TUTORIAL VIDEOS GALLERY / PLAYLIST SECTION -->
                <div class="mt-4 pt-2">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                        <div>
                            <h5 class="text-white fw-bold mb-0">
                                <i class="fas fa-th-large text-info me-2"></i> Tutorial Guides Playlist
                            </h5>
                            <span class="text-muted" style="font-size: 12px;">Showing {{ count($allVideos) }} step-by-step video tutorials. Click any video to watch.</span>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        @foreach ($videos as $v)
                            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12">
                                <div class="tutorial-side-card video-card-item h-100 d-flex flex-column overflow-hidden {{ $selectedVideo && $selectedVideo->id === $v->id ? 'active-playing-card' : '' }}" 
                                     id="video-card-{{ $v->id }}"
                                     onclick="playVideoDirect({{ json_encode([
                                         'id' => $v->id,
                                         'title' => $v->title,
                                         'tag' => $v->tag ?: 'Official Tutorial',
                                         'duration' => $v->duration ?: 'Step-by-Step Guide',
                                         'url' => $v->video_play_url,
                                         'embed_url' => $v->embed_url,
                                         'is_youtube' => $v->isYouTube(),
                                         'download_url' => route('member.tutorial-video.download', ['filename' => $v->id]),
                                    ...
                                    <div class="position-relative overflow-hidden video-thumb-box" style="aspect-ratio: 16/9; background: #0b0f19;">
                                        ...
                                        <span class="position-absolute bottom-0 end-0 m-2 badge bg-black bg-opacity-75 text-white" style="font-size: 10.5px;">
                                            <i class="far fa-clock me-1"></i> {{ $v->duration ?: 'Guide' }}
                                        </span>
                                    </div>
                                        ...
                                        <div class="d-flex align-items-center justify-content-between pt-2 border-top border-secondary-subtle mt-2">
                                            ...
                                            @else
                                                <span class="video-source-pill pill-mp4">
                                                    <i class="fas fa-file-video text-info"></i> {{ strtoupper($v->video_file ? pathinfo($v->video_file, PATHINFO_EXTENSION) : 'HD') }} Video
                                                </span>
                                            @endif
                                            <button type="button" class="btn btn-sm btn-outline-info py-1 px-2" style="font-size: 11px; border-radius: 6px;">
                                                <i class="fas fa-play me-1"></i> Watch Guide
                                            </button>
                                        </div>
                                    </div>
                                </div>
    ...
    <style>
        .tutorial-theater-card {
            background: linear-gradient(135deg, rgba(16, 26, 48, 0.95) 0%, rgba(10, 18, 32, 0.95) 100%);
        ...
        .btn-download-tutorial:hover {
            background: linear-gradient(135deg, #33c2ff 0%, #00ff88 100%);
            color: #000;
    ...
    <script>
        ...
        function playVideoDirect(item) {
            ...
            document.getElementById('theaterDuration').innerHTML = '<i class="fas fa-clock text-info me-1"></i> ' + item.duration;
            ...
            const html5Player = document.getElementById('mainTutorialVideo');
            ...
            if (item.can_download) {
                dlContainer.innerHTML = '<a href="' + item.download_url + '" class="btn btn-download-tutorial d-flex align-items-center fw-bold"><i class="fas fa-download me-2"></i> Download Tutorial (HD)</a>';
            } else {
            ...
            document.getElementById('tutTheaterSection').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
