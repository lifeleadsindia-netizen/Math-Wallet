...
@section('title', 'Fund Requests History')
@section('content')
@push('head')
...
<div class="container-fluid">
        ...
        <div class="row align-items-end">
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        <h5>{{ __('Fund Requests History')}}</h5>
                        <span>{{ __('All Fund Requests History ')}}</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <nav class="breadcrumb-container" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ __('Fund Requests History')}}</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            @endif
            <div class="card">
                <div class="card-header"><h3>{{ __('Fund Requests History')}}</h3></div>
                    ...
                    <table id="data_table" class="table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('Member Id')}}</th>
                                <th>{{ __('Amount')}}</th>
                                <th style="width:100px">{{ __('Transfer Id')}}</th>
                                <th>{{ __('Image')}}</th>
                                <th>{{ __('Status')}}</th>
                                <th>{{ __('View')}}</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $list )
                                <tr>
                                    <td>{{$i}}</td>
                                    <td>{{ date('d-m-Y', strtotime($list['updated_at']))}}<br>{{ date('H:i:s', strtotime($list['updated_at']))}}</td>
                                    <td>{{ $list['memberid']}}</td>
                                    <td>$ {{ $list['amount']}}</td>
